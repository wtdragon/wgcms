...
class smartideo{
    ...
    private function get_iframe($url = '', $source = '', $width = '', $height = ''){
        ...
        if($this->strategy == 1){
            if(is_home())
            {
                $html .= sprintf('<link rel="stylesheet" id="smartideo-cssdd" href="%1$s" type="text/css" media="screen">', SMARTIDEO_URL . '/static/front_smartideo.css?ver=' . SMARTIDEO_VERSION);
            }
           else{
    ...
    public function smartideo_scripts(){
        if(is_home())
        {
            wp_enqueue_style('smartideo_css', SMARTIDEO_URL . '/static/front_smartideo.css', array(), SMARTIDEO_VERSION, 'screen');
        }
        else{
